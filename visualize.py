...
dataset = Plain_Dataset(csv_file=args.data+'/fer2013.csv',img_dir = args.data+'/'+'train/',datatype = 'train',transform = transformation)
...
if args.test_acc:
    with torch.no_grad():
        for data, labels in test_loader:
            ...
            correct_predictions = torch.sum(class_preds == labels).item()
    ...
    # Calcular precisión total (Precision)
    from sklearn.metrics import precision_score
    precision = precision_score(all_labels, all_preds, average='weighted')

    # Imprimir los resultados
    print(f'Accuracy of the network on the test images: {avg_accuracy:.2f} %')
